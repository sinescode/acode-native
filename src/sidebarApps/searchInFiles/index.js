...
async function searchAll() {
	...
	// Try Rust-native accelerator first
	if (typeof acode !== 'undefined' && acode.native && (await acode.native.isAvailable())) {
		try {
			await searchNative(search, options);
			return;
		} catch (nativeErr) {
			console.warn('Native search failed, falling back to workers:', nativeErr);
			// Reset state for worker fallback
			searching = true;
			words.length = 0;
			fileNames.length = 0;
			filesSearched.length = 0;
...
/**
 * Performs search using the Rust-native accelerator.
 * Iterates over all added folders and searches each one via acode.native.searchFiles().
 * Results are processed inline, bypassing the Web Worker infrastructure.
 * @param {string} search
 * @param {Options} options
 */
async function searchNative(search, options) {
	const roots = addedFolder.map(f => f.url);
	for (const root of roots) {
		try {
			const nativeResults = await acode.native.searchFiles(root, search, {
				caseSensitive: options.caseSensitive,
				wholeWord: options.wholeWord,
				regexMode: options.regExp,
				exclude: options.exclude,
				include: options.include,
			});

			if (!nativeResults || !nativeResults.length) continue;

			for (const result of nativeResults) {
				const { file, matches, text } = result;
				if (!matches || !matches.length) continue;

				const fileObj = Tree.fromJSON(file);
				if (filesSearched.find(f => f.url === fileObj.url)) continue;

				filesSearched.push(fileObj);
				if (filesSearched.length === 1) {
					searchResult.setValue("");
				}
				resultOverview.filesCount += 1;
				...
				for (const match of matches) {
					...
					results.push(match);
					if (words.length < MAX_HL_WORDS) {
						const token = escapeStringRegexp(match.renderText);
						if (!words.includes(token)) words.push(token);
					}
				}
			...
			console.warn(`Native search failed for root ${root}:`, err);
		}
	}
