...
export default async function openFile(file, options = {}) {
	try {
		...
		if (typeof acode !== 'undefined' && acode.native && (await acode.native.isAvailable())) {
			try {
				const nativeEncoding = file.encoding || encoding || appSettings.value.defaultFileEncoding || 'auto';
				const nativeResult = await acode.native.fsReadFile(uri, nativeEncoding);
				const binStr = atob(nativeResult.data_base64);
				const bytes = new Uint8Array(binStr.length);
				for (let i = 0; i < binStr.length; i++) {
					bytes[i] = binStr.charCodeAt(i);
				}
				const fileContent = await decode(bytes.buffer, nativeResult.encoding || 'UTF-8');
				createEditor(false, fileContent, nativeResult.encoding || 'UTF-8');
				if (mode !== 'single') recents.addFile(uri);
				return;
			} catch (nativeErr) {
				console.warn('Native fsReadFile failed, falling back:', nativeErr);
			}
		}
