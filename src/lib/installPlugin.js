...
export default async function installPlugin(
	...
	try {
		...
		if (plugin) {
			...
			let nativeExtracted = false;
			if (typeof acode !== 'undefined' && acode.native && (await acode.native.isAvailable())) {
				try {
					await acode.native.zipExtract(plugin, pluginDir);
					// Write patched plugin.json over the extracted one
					await fsOperation(Url.join(pluginDir, 'plugin.json')).writeFile(JSON.stringify(pluginJson));
					// Register all files in install state
					const allZipEntries = Object.keys(zip.files);
					for (const zfile of allZipEntries) {
						let correctFile = zfile.replace(/\\/g, '/');
						if (correctFile.endsWith('/')) continue;
						if (isUnsafeAbsolutePath(zfile)) continue;
						correctFile = sanitizeZipPath(correctFile, false);
						if (!correctFile) continue;
						state.updatedStore[correctFile.toLowerCase()] = 'native';
					}
					nativeExtracted = true;
				} catch (nativeErr) {
					console.warn('Native zip extraction failed, falling back to JSZip:', nativeErr);
				}
			}
