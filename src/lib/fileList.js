...
async function getAllFiles(parent, root) {
	...
	// Try Rust-native accelerator first (only at root level to avoid duplicate work)
	if (parent === root && typeof acode !== 'undefined' && acode.native && (await acode.native.isAvailable())) {
		try {
			const nativeTree = await acode.native.fsWalkTree(parent.url);
			if (nativeTree && nativeTree.children) {
				await importNativeTree(parent, nativeTree, root);
				return;
			}
		} catch (nativeErr) {
			console.warn('Native fsWalkTree failed, falling back to lsDir:', nativeErr);
		}
	}
...
/**
 * Recursively imports a native file tree (from acode.native.fsWalkTree) into
 * the application's Tree structure. Mirrors createChildTree logic but operates
 * on a pre-built tree instead of individual lsDir entries.
 * @param {Tree} parent - Parent Tree node to populate
 * @param {object} nativeNode - Native tree node with { name, url, isDirectory, children }
 * @param {Tree} root - Root Tree node for connection checking
 */
async function importNativeTree(parent, nativeNode, root) {
	if (!root.isConnected) return;
	if (!nativeNode.children) return;

	for (const child of nativeNode.children) {
		if (!root.isConnected) return;

		const { name, url, isDirectory } = child;
		const exists = parent.children.findIndex(({ value }) => value === url);
		...
		if (isDirectory) {
			...
			await importNativeTree(file, child, root);
		} else {
			emit("push-file", file);
		}
	}
